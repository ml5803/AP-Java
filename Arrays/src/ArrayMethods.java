...
public class ArrayMethods {
	...
	public static int[] generateRandomExtensions(int[] intList){

		int[] randArr = new int[intList.length-2];
		for (int i = 0; i<intList.length; i++){
			int rand = (int) ((double)intList.length * Math.random());
			while(searchUnsorted(randArr,intList[rand])>-1){
				rand = (int) ((double)intList.length * Math.random());
				//System.out.println("The test number was "+ rand);
			}
			randArr[i]=intList[rand];
		}
		return randArr;
